    ...
    */
    ...
    value: function receiveRequest(request) {
        ...
        switch (method) {
          ...
          case JsSIP_C.CANCEL:
            ...
            if (session) {
              ...
              this._traceUnmatched('cancel_rx_unmatched', request);
            }
            break;
          case JsSIP_C.ACK:
            /* Absorb it.
      ...
      // In-dialog request.
         ...
         * been created.
         */else {
          this._traceUnmatched('rx_unmatched_indialog', request);
    ...
    /**
     * Compact SIP signaling trace for requests that match no session/dialog.
     * Mirrors RTCSession#_trace: metadata only, zero-cost without a listener.
     */
  }, {
    key: "_traceUnmatched",
    value: function _traceUnmatched(type, request) {
      if (this.listenerCount('siptrace') === 0) {
        return;
      }
      try {
        this.emit('siptrace', {
          type: type,
          call_id: request && request.call_id || null,
        ...
        debugerror('_traceUnmatched() failed: %o', error);
      }
    }
