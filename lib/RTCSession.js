...
module.exports = class RTCSession extends EventEmitter {
  ...
  init_incoming(request, initCallback) {
    ...
    this._trace('invite_rx', { late_sdp: this._late_sdp });
  ...
  answer(options = {}) {
    ...
    if (this.replyObject) {
      ...
      if (this._status !== C.STATUS_ANSWERED) {
        const staleStatus = this._status;

        delete this.replyObject;
        this._trace('answer_stale_reply', { stale_status: staleStatus });
    ...
    Promise.resolve()
      ...
      .then((desc) => {
        ...
        const replyObject = { 'code': 200, 'request': request, 'extraHeaders': extraHeaders, 'desc': desc };

        if (!options.preAnswer || this.answeredEarly) {
          this.reply(replyObject);
        }
        else {
          debug('This is a preAnswer() call');
          this.replyObject = replyObject;
          this._trace('answer_ready');
          this.emit('answerReady', replyObject);
        }
  ...
  reply(options = {}) {
    ...
    this._trace('tx_2xx_attempt', { code });

    request.reply(code, null, extraHeaders,
      desc,
      () => {
        this._status = C.STATUS_WAITING_FOR_ACK;
        ...
        this._setInvite2xxTimer(request, desc);
        this._setACKTimer();
        this._trace('tx_2xx_sent', { code });
        this._accepted('local');
      },
      () => {
        this._trace('tx_2xx_transport_fail', { code });
        this._failed('system', null, JsSIP_C.causes.CONNECTION_ERROR);
      }
    );
  }
    ...
    switch (this._status) {
      ...
      case C.STATUS_1XX_RECEIVED:
        ...
        if (this._status === C.STATUS_NULL || this._status === C.STATUS_INVITE_SENT) {
          ...
          this._trace('cancel_tx', { deferred: true });
        }
        else if (this._status === C.STATUS_1XX_RECEIVED) {
          this._trace('cancel_tx', { deferred: false });
      ...
      case C.STATUS_ANSWERED:
        ...
        this._trace('tx_reject', { code: status_code });
        ...
        if (this._status === C.STATUS_WAITING_FOR_ACK &&
          ...
          this.receiveRequest = ({ method }) => {
            if (method === JsSIP_C.ACK) {
              this._trace('bye_tx', { deferred: true });
          ...
          this._trace('bye_tx', { deferred: false });
          this.sendRequest(JsSIP_C.BYE, {
            extraHeaders,
            body
          });
  ...
  receiveRequest(request) {
    ...
    if (request.method === JsSIP_C.CANCEL) {
      ...
      */
      const cancelProcessed = this._status === C.STATUS_WAITING_FOR_ANSWER ||
        this._status === C.STATUS_ANSWERED;

      this._trace('cancel_rx', { processed: cancelProcessed });
      ...
      switch (request.method) {
        case JsSIP_C.ACK:
          this._trace('ack_rx', { processed: this._status === C.STATUS_WAITING_FOR_ACK });
        ...
        case JsSIP_C.BYE:
          if (this._status === C.STATUS_CONFIRMED ||
            this._status === C.STATUS_WAITING_FOR_ACK) {
            this._trace('bye_rx', { processed: true });
            request.reply(200);
            ...
            this._trace('bye_rx', { processed: false });
            request.reply(403, 'Wrong Status');
          }
          break;
        case JsSIP_C.INVITE:
          if (this._status === C.STATUS_CONFIRMED) {
            this._trace('reinvite_rx', { replaces: request.hasHeader('replaces') });
            ...
            this._trace('reinvite_rx', { processed: false });
            request.reply(403, 'Wrong Status');
          }
          break;
        case JsSIP_C.INFO:
        ...
        case JsSIP_C.UPDATE:
          if (this._status === C.STATUS_CONFIRMED) {
            ...
            this._trace('update_rx', { processed: false });
            request.reply(403, 'Wrong Status');
          }
          break;
        case JsSIP_C.REFER:
          if (this._status === C.STATUS_CONFIRMED) {
            ...
            this._trace('refer_rx', { processed: false });
            request.reply(403, 'Wrong Status');
          }
          break;
        case JsSIP_C.NOTIFY:
  ...
  _setInvite2xxTimer(request, body) {
    ...
    function invite2xxRetransmission() {
      ...
      attempts += 1;
      this._trace('tx_2xx_retrans', { attempt: attempts });
  ...
  _receiveInviteResponse(response) {
    ...
    this._trace('invite_response', { code: response.status_code });
  ...
  _confirmed(originator, ack) {
    ...
    this._trace('confirmed', { originator });
  ...
  _ended(originator, message, cause) {
    ...
    this._trace('ended', { originator, cause });
  ...
  _failed(originator, message, cause) {
    ...
    this._trace('failed', { originator, cause });
