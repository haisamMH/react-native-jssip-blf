...
module.exports = class RTCSession extends EventEmitter {
   ...
   */
  _getDTMFRTPSender() {
    const sender = this._connection.getLocalStreams()[0]
      .getAudioTracks().find((rtpSender) => {
  ...
  _toggleMuteAudio(mute) {
    const senders = this._connection.getLocalStreams()[0]
      .getAudioTracks().filter((track) => {
